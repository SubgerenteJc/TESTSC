...
CREATE proc [dbo].[sp_compCartaPortev4_conceptosfac]  @lgh_hdrnumber varchar(20)
...
declare  @matpeligroso  integer, @conceptoscuantos integer
...
TT_cht_itemcode varchar(6),   TT_ivd_description varchar(60),   TT_cmp_id varchar(8),   TT_cmp_name varchar(100),   TT_cty_nmstct varchar(25),   
TT_ivd_quantity float,   TT_ivd_rate money,   TT_ivd_charge money,   TT_ivd_billto varchar(8),   TT_ivh_hdrnumber int,   
TT_ivd_number int,   TT_ord_hdrnumber int,   TT_ivd_glnum char(32),   TT_ivd_type varchar(6),   TT_ivd_unit varchar(6),   
TT_cur_code varchar(6),   TT_ivd_currencydate datetime,   TT_ivd_rateunit varchar(6),   TT_ivd_sequence int,   TT_ivd_invoicestatus varchar(6),   
TT_ivd_refnum varchar(30),            TT_cmd_code varchar(8),            TT_ivd_reftype varchar(6),            TT_ivd_sign smallint,            TT_cht_basis varchar(6),
TT_cht_basisunit varchar(6),            TT_cmd_taxtable1 char(1),            TT_cmd_taxtable2 char(1),            TT_cmd_taxtable3 char(1),            TT_cmd_taxtable4 char(1),   
TT_ivd_taxable1 char(1),            TT_ivd_taxable2 char(1),            TT_ivd_taxable3 char(1),            TT_ivd_taxable4 char(1),         TT_ivd_fromord char(1),
TT_tar_number int,         TT_tar_tariffnumber varchar(12),         TT_tar_tariffitem varchar(12),         TT_ivd_remark varchar(255),         TT_stp_number int,
TT_cht_class varchar(6),         TT_cht_rateprotect char(1),         TT_cht_primary char(1),         TT_cht_rollintolh int,         TT_cht_lh_rev char(1),
TT_cht_lh_min char(1),         TT_cht_lh_stl char(1),         TT_cht_lh_rpt char(1),         TT_ivd_paylgh_number int,         TT_ivd_tariff_type char(1),
TT_ivd_taxid varchar(15),         TT_gp_tax int,         TT_ivd_charge_type smallint,          TT_cht_lh_prn char(1),	TT_ivd_revtype1 varchar(6),
TT_revtype1_t varchar(10),	TT_chrgtypeclass_t varchar(18),	TT_ivd_wgt float,	TT_ivd_wgtunit varchar(6),	TT_ivd_count dec,
TT_ivd_countunit varchar(6),	TT_ivd_volume float,	TT_ivd_volunit varchar(6),	TT_ivd_distance float,	TT_ivd_distunit varchar(6),
TT_ivh_invoicenumber varchar(12),	TT_ivh_definition  varchar(6) ,	TT_c_approved_protect int,	TT_ivd_hide char(1),	TT_usr_supervisor char(1),
TT_ivd_payrevenue money,    TT_fgt_number int)

-- lee el numero de movimiento
select @li_num_mov = mov_number from legheader where lgh_number = @lgh_hdrnumber;

insert into #conceptosfactura execute dbo.d_ord_accessorials_by_mov_sp_JR   @mov = @li_num_mov
...
TT2_remark varchar(254),TT2_cantidad float, TT2_rate money,TT2_unidadSat varchar(50),TT2_consecutivo int identity)
	...
	select TT_cht_itemcode, cht_description, TT_ivd_description,TT_ivd_charge,
	TT_ivd_taxable1, TT_ivd_taxable2 , TT_ivd_number, cht_remark,	TT_ivd_quantity, TT_ivd_rate, cwt.UnidadSat
	from #conceptosfactura,  chargetype,catWhenThen cwt
	where TT_cht_itemcode = cht_itemcode
	and cwt.cWhen = cht_description
	order by TT_ivd_number

	-- saca el numero total de conceptos 
	select @conceptoscuantos = count(*)
...
'00'																											--1 Tipo Registro
																					+'|'+
'TDRXP' + cast(legheader.lgh_number  as varchar(20))														-- 2 'Nombre de archivo '
																					+'|'+
...
--SECCION RECEPTOR (1:1)
	...
	from company where cmp_id = orderheader.ord_billto),'')                                                           --11 Direccion (R)  	 
                                                                               +'|'+     
    isnull((select replace((select stc_state_desc from  statecountry
	 where stc_State_c = cmp_state),'|','') from company where cmp_id = orderheader.ord_billto),'')                  --12 Municiipio Delegacion (R)  
																		       +'|'+     
    rtrim(isnull((select replace((select stc_state_alt from statecountry where stc_state_c = cmp_state),'|','') 
	from company where cmp_id = orderheader.ord_billto),'')  )                                                       --13 Estado (R)  
	                                                                           +'|'+  
																			   
    isnull((select replace(cmp_zip  ,'|','') from company where cmp_id = orderheader.ord_billto),'')                 --14 CP (R)     
																		       +'|'+                        
	...
	''                                                                                                               --17 Correo electronio para envio
                                                                               +'|'+   
    isnull((select replace(cmp_volunits  ,'|','') from company where cmp_id = orderheader.ord_billto),'601') 			 --18 Regimen fiscal receptor
																			   +'|'+
                                                                               + '\n' +
...
----SECCION 04 (1:N)
	...
	''																										-- 12 Importe con IVA
																				+'|'+
	'02'--objeto impuesto'																				--13 objeto impuesto
																				+'|'+	   
                                                                               + '\n' +
...
+
 case when @conceptoscuantos = 0 then
 ''
 else
	replace( (STUFF(( 
...
--SECCION  FIGURA TRANSPORTE (0:N)
    ...
    'CP2_TIP_FIG'                                                           --1 Tipo de Registro
																	        +'|'+ 
	 isnull(REPLACE(orderheader.ord_refnum,'|',''),'')						--2 Consecutivo tipo figura transporte -- se ocupo para la referencia JR
																	        +'|'+ 
