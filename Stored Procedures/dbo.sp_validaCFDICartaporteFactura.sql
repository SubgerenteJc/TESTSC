...
CREATE proc [dbo].[sp_validaCFDICartaporteFactura] @lgh_hdrnumber varchar(20)
as

declare @primerLetra char(1),
		@lgh_hdrnumber2 varchar(20),
		@num_factura int,
		@num_movimiento int
		...
		select 
		  ...
		  case when 
		   ...
		   (select 
			...
			from freightdetail where stp_number in  (select stp_number from stops where stp_type  ='PUP'  and lgh_number= @lgh_hdrnumber2))  <= 0 then 'Peso del segmento ' + @lgh_hdrnumber2 + '<b style='+''''+'color:red;'+''''+'> no valido^</b>' else 'Peso segmento ' + @lgh_hdrnumber2 + ' Ok' end  + '<br>'+ 
